module.exports = function (restClient) {
  ...
  module.awrafData = function (customerToken) {
    url += `awrafData?token=${customerToken}`;
    return restClient.get(url).then((data)=> {
        return getResponse(data);
    });
  }
  module.awrafCoupon = function (awrafData) {
    url += `awrafCoupon?token=${awrafData.token}&cartId=${awrafData.cartId}`;
    return restClient.post(url, {awraf_amount: awrafData.awraf_amount, remove: awrafData.remove}).then((data)=> {
        return getResponse(data);
    });
  }
  ...
  module.changeForgotten = function (passwordData) {
    url += `changeForgotten`;
    return restClient.post(url, passwordData.body).then((data)=> {
        return getResponse(data);
    });
